...
public class QuestionRepository implements IQuestionRepository {
    ...
    public long count() {
        // TODO Auto-generated method stub
        return questionMap.entrySet().stream().count();
    }
    ...
    public List<Question> findAllQuestionLevelWise(Level level) {
        return questionMap.entrySet().stream().map(e->e.getValue()).filter(e->e.getLevel()==level).collect(Collectors.toList());
    }
