...
public class CreateContestCommand implements ICommand{
    ...
    public void execute(List<String> tokens) {
        ...
        try{
            if(tokens.size()==4){
                String str=tokens.get(3);
                boolean result = str.matches("[0-9]+");
                if(result){
                    System.out.println("Contest Creator for given name: creator not found!");
                    
                }else{
                    Contest c=contestService.create(tokens.get(1), Level.valueOf(tokens.get(2)),
                    tokens.get(3), null);
                    System.out.println(c);
                }
            }
            if(tokens.get(0)=="CREATE-CONTEST" && tokens.size()==5){
                Contest c=contestService.create(tokens.get(1), Level.valueOf(tokens.get(2)),
                tokens.get(3), Integer.parseInt(tokens.get(4)));
                System.out.println(c);
            }
